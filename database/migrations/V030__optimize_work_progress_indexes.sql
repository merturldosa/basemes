...
ON mes.si_work_orders(tenant_id, assigned_user_id, status)
...
ON qms.si_sop_execution(tenant_id, work_order_id, execution_status);

-- Index for active SOP executions
CREATE INDEX IF NOT EXISTS idx_sop_execution_active
ON qms.si_sop_execution(tenant_id, execution_status, execution_date DESC)
...
ON qms.si_sop_execution_step(execution_id, step_number);

-- Index for failed SOP steps
CREATE INDEX IF NOT EXISTS idx_sop_exec_step_failed
ON qms.si_sop_execution_step(execution_id, step_status, check_result)
...
ON mes.si_work_result(tenant_id, work_order_id, completion_date DESC);

-- Index for operator performance analytics
CREATE INDEX IF NOT EXISTS idx_work_result_operator
ON mes.si_work_result(tenant_id, operator_user_id, completion_date DESC);

-- Index for daily production statistics
CREATE INDEX IF NOT EXISTS idx_work_result_daily_stats
ON mes.si_work_result(tenant_id, completion_date, product_id)
...
ON wms.si_inventory(tenant_id, product_id, warehouse_id, lot_id)
WHERE is_deleted = FALSE;

-- Index for recent inventory transactions
CREATE INDEX IF NOT EXISTS idx_inventory_txn_recent
ON wms.si_inventory_transaction(tenant_id, transaction_date DESC, transaction_type)
WHERE transaction_date >= CURRENT_DATE - INTERVAL '7 days';

-- Composite index for POP dashboard queries
CREATE INDEX IF NOT EXISTS idx_pop_dashboard_composite
ON mes.si_work_progress(tenant_id, record_date, status, operator_user_id)
...
ANALYZE mes.si_work_progress;
ANALYZE mes.si_pause_resume_history;
ANALYZE mes.si_work_orders;
ANALYZE mes.si_work_result;
ANALYZE qms.si_defects;
ANALYZE qms.si_sop_execution;
ANALYZE qms.si_sop_execution_step;
ANALYZE wms.si_inventory;
ANALYZE wms.si_inventory_transaction;
