...
public class MaterialRequestController {
    ...
    public ResponseEntity<ApiResponse<List<MaterialRequestResponse>>> getMaterialRequests(
        ...
        if (status != null) {
            ...
            requests = materialRequestService.findByWorkOrder(workOrderId);
        } else {
            requests = materialRequestService.findByTenant(tenantId);
        }

        // Additional filtering by warehouse or requester if needed
        if (warehouseId != null) {
            requests = requests.stream()
                .filter(r -> r.getWarehouse().getWarehouseId().equals(warehouseId))
                .collect(Collectors.toList());
        }
        if (requesterId != null) {
            requests = requests.stream()
                .filter(r -> r.getRequester().getUserId().equals(requesterId))
                .collect(Collectors.toList());
        }
    ...
    public ResponseEntity<ApiResponse<MaterialRequestResponse>> approveMaterialRequest(
        ...
        MaterialRequestEntity approved = materialRequestService.approveMaterialRequest(id, approverUserId, null);
    ...
    private MaterialRequestResponse toMaterialRequestResponse(MaterialRequestEntity entity) {
        return MaterialRequestResponse.builder()
                ...
                .approvedDate(entity.getApprovalDate())
                // Dates
